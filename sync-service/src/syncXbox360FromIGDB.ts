/**
 * IGDB platform sync: fetches all non-cancelled Xbox 360 games from IGDB
 * and upserts one DB row per (game × region) for EU / NA / JP whenever IGDB
 * has a regional release date for that combination.
 *
 * Usage:
 *   npx tsx src/syncXbox360FromIGDB.ts              # dry-run (no DB writes)
 *   DRY_RUN=false npx tsx src/syncXbox360FromIGDB.ts # upsert to Supabase
 *   LIMIT=50 npx tsx src/syncXbox360FromIGDB.ts      # process only first 50
 */
...
async function main() {
  console.log('🎮 Xbox 360 IGDB platform sync');
  ...
  console.log(`✅ Release dates fetched (${rdMap.size} games have Xbox 360 dates)\n`);
