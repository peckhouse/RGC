/**
 * IGDB platform sync: fetches all non-cancelled NES and Famicom games from
 * IGDB and upserts one DB row per (game × region) for EU / NA / JP.
 *
 * Nintendo Entertainment System (platform 18) and Family Computer / Famicom
 * (platform 99) are treated as a single platform group — games from both are
 * deduped by IGDB game ID. Many JP-only titles are tagged exclusively under
 * the Famicom platform and would be missed with platform 18 alone.
 ...
 *
 * Usage:
 *   npx tsx src/syncNESFromIGDB.ts              # dry-run (no DB writes)
 *   DRY_RUN=false npx tsx src/syncNESFromIGDB.ts # upsert to Supabase
 *   LIMIT=50 npx tsx src/syncNESFromIGDB.ts      # process only first 50
 *
...
/** Nintendo Entertainment System IGDB platform ID */
const NES_PLATFORM_ID = 18;
/** Family Computer (Famicom) IGDB platform ID */
const FAMICOM_PLATFORM_ID = 99;
const PLATFORM_IDS = [NES_PLATFORM_ID, FAMICOM_PLATFORM_ID];
...
async function main() {
  console.log('🎮 NES / Famicom IGDB platform sync');
  ...
  //    Many JP-only Famicom titles are tagged exclusively under platform 99
  //    and would be missed if we queried platform 18 alone.
  ...
  console.log(`📊 Total unique games (NES + Famicom combined): ${games.length}\n`);
  ...
  for (const game of games) {
    ...
    // First date encountered wins per region (merged from NES + Famicom).
