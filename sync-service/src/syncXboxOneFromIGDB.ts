/**
 * IGDB platform sync: fetches all non-cancelled Xbox One games from IGDB
 * and upserts one DB row per (game x region) for EU / NA / JP whenever IGDB
 * has a regional release date for that combination.
 *
 * Usage:
 *   npx tsx src/syncXboxOneFromIGDB.ts              # dry-run (no DB writes)
 *   DRY_RUN=false npx tsx src/syncXboxOneFromIGDB.ts # upsert to Supabase
 *   LIMIT=50 npx tsx src/syncXboxOneFromIGDB.ts      # process only first 50
 */
...
/** Xbox One IGDB platform ID */
const PLATFORM_ID = 49;
...
async function main() {
  console.log('🎮 Xbox One IGDB platform sync');
  ...
  console.log(`✅ Release dates fetched (${rdMap.size} games have Xbox One dates)\n`);
