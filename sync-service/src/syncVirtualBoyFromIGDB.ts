/**
 * IGDB platform sync: fetches all non-cancelled Virtual Boy games from IGDB
 * and upserts one DB row per (game x region) for EU / NA / JP whenever IGDB
 * has a regional release date for that combination.
 *
 * Usage:
 *   npx tsx src/syncVirtualBoyFromIGDB.ts              # dry-run (no DB writes)
 *   DRY_RUN=false npx tsx src/syncVirtualBoyFromIGDB.ts # upsert to Supabase
 *   LIMIT=50 npx tsx src/syncVirtualBoyFromIGDB.ts      # process only first 50
 */
...
/** Virtual Boy IGDB platform ID */
const PLATFORM_ID = 87;
...
async function main() {
  console.log('🎮 Virtual Boy IGDB platform sync');
  ...
  console.log(`✅ Release dates fetched (${rdMap.size} games have Virtual Boy dates)\n`);
