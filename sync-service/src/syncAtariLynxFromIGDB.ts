/**
 * IGDB platform sync: fetches all non-cancelled Atari Lynx games from IGDB
 * and upserts one DB row per (game × region) for EU / NA / JP whenever IGDB
 * has a regional release date for that combination.
 *
 * Usage:
 *   npx tsx src/syncAtariLynxFromIGDB.ts              # dry-run (no DB writes)
 *   DRY_RUN=false npx tsx src/syncAtariLynxFromIGDB.ts # upsert to Supabase
 *   LIMIT=50 npx tsx src/syncAtariLynxFromIGDB.ts      # process only first 50
 */
...
/** Atari Lynx IGDB platform ID */
const PLATFORM_ID = 61;
...
async function main() {
  console.log('🎮 Atari Lynx IGDB platform sync');
  ...
  console.log(`✅ Release dates fetched (${rdMap.size} games have Atari Lynx dates)\n`);
