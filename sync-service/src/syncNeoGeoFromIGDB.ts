/**
 * IGDB platform sync: fetches all non-cancelled Neo Geo games from IGDB
 * and upserts one DB row per (game × region) for EU / NA / JP whenever IGDB
 * has a regional release date for that combination.
 *
 * Note: "Neo Geo AES" and "Neo Geo MVS" are aliased to "Neo Geo" in
 * syncConsoles.ts. Games fetched here use the canonical Neo Geo platform ID.
 *
 * Usage:
 *   npx tsx src/syncNeoGeoFromIGDB.ts              # dry-run (no DB writes)
 *   DRY_RUN=false npx tsx src/syncNeoGeoFromIGDB.ts # upsert to Supabase
 *   LIMIT=50 npx tsx src/syncNeoGeoFromIGDB.ts      # process only first 50
 */
...
/** Neo Geo IGDB platform ID */
const PLATFORM_ID = 80;
...
async function main() {
  console.log('🎮 Neo Geo IGDB platform sync');
  ...
  console.log(`✅ Release dates fetched (${rdMap.size} games have Neo Geo dates)\n`);
