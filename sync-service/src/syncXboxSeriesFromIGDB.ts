/**
 * IGDB platform sync: fetches all non-cancelled Xbox Series X and Xbox Series S
 * games from IGDB and upserts one DB row per (game × region) for EU / NA / JP.
 *
 * Xbox Series X (platform 169) and Xbox Series S (platform 170) are treated as
 * a single platform group — games from both are deduped by IGDB game ID.
 * Release dates are collected from both platform IDs and merged.
 *
 * Usage:
 *   npx tsx src/syncXboxSeriesFromIGDB.ts              # dry-run (no DB writes)
 *   DRY_RUN=false npx tsx src/syncXboxSeriesFromIGDB.ts # upsert to Supabase
 *   LIMIT=50 npx tsx src/syncXboxSeriesFromIGDB.ts      # process only first 50
 */
...
/** Xbox Series X IGDB platform ID */
const SERIES_X_PLATFORM_ID = 169;
/** Xbox Series S IGDB platform ID */
const SERIES_S_PLATFORM_ID = 170;
const PLATFORM_IDS = [SERIES_X_PLATFORM_ID, SERIES_S_PLATFORM_ID];
...
async function main() {
  console.log('🎮 Xbox Series X|S IGDB platform sync');
  ...
  const platformLabels: Record<number, string> = {
    [SERIES_X_PLATFORM_ID]: 'Xbox Series X',
    [SERIES_S_PLATFORM_ID]: 'Xbox Series S',
  };
  ...
  console.log(`📊 Total unique games (Xbox Series X + Series S combined): ${games.length}\n`);
