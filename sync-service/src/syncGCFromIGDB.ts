/**
 * IGDB platform sync: fetches all non-cancelled Nintendo GameCube games from
 ...
 *
 * Usage:
 *   npx tsx src/syncGCFromIGDB.ts              # dry-run (no DB writes)
 *   DRY_RUN=false npx tsx src/syncGCFromIGDB.ts # upsert to Supabase
 *   LIMIT=50 npx tsx src/syncGCFromIGDB.ts      # process only first 50
 *
...
/** Nintendo GameCube IGDB platform ID */
...
async function main() {
  console.log('🎮 Nintendo GameCube IGDB platform sync');
  ...
  // 1. Paginate through all GameCube games on IGDB
  ...
  // 2. Batch-fetch release_dates for GameCube directly from the release_dates endpoint.
  ...
  console.log(`✅ Release dates fetched (${rdMap.size} games have GameCube dates)\n`);
