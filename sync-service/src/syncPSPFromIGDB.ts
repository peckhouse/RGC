/**
 * IGDB platform sync: fetches all non-cancelled PlayStation Portable games from IGDB
 * and upserts one DB row per (game × region) for EU / NA / JP whenever IGDB
 * has a regional release date for that combination.
 *
 * Usage:
 *   npx tsx src/syncPSPFromIGDB.ts              # dry-run (no DB writes)
 *   DRY_RUN=false npx tsx src/syncPSPFromIGDB.ts # upsert to Supabase
 *   LIMIT=50 npx tsx src/syncPSPFromIGDB.ts      # process only first 50
 */
...
/** PlayStation Portable (PSP) IGDB platform ID */
const PLATFORM_ID = 38;
...
async function main() {
  console.log('🎮 PlayStation Portable (PSP) IGDB platform sync');
  ...
  console.log(`✅ Release dates fetched (${rdMap.size} games have PSP dates)\n`);
