/**
 * IGDB platform sync: fetches all non-cancelled Xbox games from IGDB
 * and upserts one DB row per (game × region) for EU / NA / JP whenever IGDB
 * has a regional release date for that combination.
 *
 * Usage:
 *   npx tsx src/syncXboxFromIGDB.ts              # dry-run (no DB writes)
 *   DRY_RUN=false npx tsx src/syncXboxFromIGDB.ts # upsert to Supabase
 *   LIMIT=50 npx tsx src/syncXboxFromIGDB.ts      # process only first 50
 */
...
/** Xbox (original) IGDB platform ID */
const PLATFORM_ID = 11;
...
async function main() {
  console.log('🎮 Xbox IGDB platform sync');
  ...
  console.log(`✅ Release dates fetched (${rdMap.size} games have Xbox dates)\n`);
