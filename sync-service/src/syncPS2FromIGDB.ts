/**
 * IGDB platform sync: fetches all non-cancelled PlayStation 2 games from IGDB
 * and upserts one DB row per (game × region) for EU / NA / JP whenever IGDB
 * has a regional release date for that combination.
 *
 * Usage:
 *   npx tsx src/syncPS2FromIGDB.ts              # dry-run (no DB writes)
 *   DRY_RUN=false npx tsx src/syncPS2FromIGDB.ts # upsert to Supabase
 *   LIMIT=50 npx tsx src/syncPS2FromIGDB.ts      # process only first 50
 */
...
/** PlayStation 2 IGDB platform ID */
const PLATFORM_ID = 8;
...
async function main() {
  console.log('🎮 PlayStation 2 IGDB platform sync');
  ...
  console.log(`✅ Release dates fetched (${rdMap.size} games have PS2 dates)\n`);
