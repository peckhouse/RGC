/**
 * IGDB platform sync: fetches all non-cancelled Atari 2600 and Atari 7800
 * games from IGDB and upserts one DB row per (game × region) for EU / NA / JP.
 *
 * Atari 2600 (platform 59) and Atari 7800 (platform 60) are treated as a single
 * platform group — the 7800 is backward-compatible with 2600 cartridges and both
 * are merged into the "Atari 2600 / 7800" console via PLATFORM_ALIASES in
 * syncConsoles.ts. Games from both are deduped by IGDB game ID.
 *
 * Usage:
 *   npx tsx src/syncAtari2600FromIGDB.ts              # dry-run (no DB writes)
 *   DRY_RUN=false npx tsx src/syncAtari2600FromIGDB.ts # upsert to Supabase
 *   LIMIT=50 npx tsx src/syncAtari2600FromIGDB.ts      # process only first 50
 */
...
/** Atari 2600 IGDB platform ID */
const ATARI2600_PLATFORM_ID = 59;
/** Atari 7800 IGDB platform ID */
const ATARI7800_PLATFORM_ID = 60;
const PLATFORM_IDS = [ATARI2600_PLATFORM_ID, ATARI7800_PLATFORM_ID];
...
async function main() {
  console.log('🎮 Atari 2600 / 7800 IGDB platform sync');
  ...
  const platformLabels: Record<number, string> = {
    [ATARI2600_PLATFORM_ID]: 'Atari 2600',
    [ATARI7800_PLATFORM_ID]: 'Atari 7800',
  };
  ...
  console.log(`📊 Total unique games (Atari 2600 + 7800 combined): ${games.length}\n`);
