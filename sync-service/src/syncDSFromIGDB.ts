/**
 * IGDB platform sync: fetches all non-cancelled Nintendo DS games from IGDB
 * and upserts one DB row per (game × region) for EU / NA / JP whenever IGDB
 * has a regional release date for that combination.
 *
 * The Nintendo DSi is a hardware revision with no physical-only exclusives;
 * all DS/DSi games are tagged under the base Nintendo DS platform (20) in IGDB.
 *
 * Cover art priority:
 ...
 *
 * Usage:
 *   npx tsx src/syncNDSFromIGDB.ts              # dry-run (no DB writes)
 *   DRY_RUN=false npx tsx src/syncNDSFromIGDB.ts # upsert to Supabase
 *   LIMIT=50 npx tsx src/syncNDSFromIGDB.ts      # process only first 50
 *
...
/** Nintendo DS IGDB platform ID */
const PLATFORM_ID = 20;
...
async function main() {
  console.log('🎮 Nintendo DS IGDB platform sync');
  ...
  // 1. Paginate through all NDS games on IGDB
  ...
  // 2. Batch-fetch release_dates for NDS directly from the release_dates endpoint.
  ...
  console.log(`✅ Release dates fetched (${rdMap.size} games have NDS dates)\n`);
