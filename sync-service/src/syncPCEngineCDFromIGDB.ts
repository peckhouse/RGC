/**
 * IGDB platform sync: fetches all non-cancelled TurboGrafx-CD / PC Engine CD
 * games from IGDB and upserts one DB row per (game x region) for EU / NA / JP
 * whenever IGDB has a regional release date for that combination.
 *
 * Usage:
 *   npx tsx src/syncPCEngineCDFromIGDB.ts              # dry-run (no DB writes)
 *   DRY_RUN=false npx tsx src/syncPCEngineCDFromIGDB.ts # upsert to Supabase
 *   LIMIT=50 npx tsx src/syncPCEngineCDFromIGDB.ts      # process only first 50
 */
...
/** TurboGrafx-CD/PC Engine CD IGDB platform ID */
const PLATFORM_ID = 150;
...
async function main() {
  console.log('🎮 PC Engine CD / TurboGrafx-CD IGDB platform sync');
  ...
  console.log(`✅ Release dates fetched (${rdMap.size} games have PC Engine CD dates)\n`);
