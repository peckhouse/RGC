/**
 * IGDB platform sync: fetches all non-cancelled Atari Jaguar games from IGDB
 * and upserts one DB row per (game x region) for EU / NA / JP whenever IGDB
 * has a regional release date for that combination.
 *
 * Usage:
 *   npx tsx src/syncJaguarFromIGDB.ts              # dry-run (no DB writes)
 *   DRY_RUN=false npx tsx src/syncJaguarFromIGDB.ts # upsert to Supabase
 *   LIMIT=50 npx tsx src/syncJaguarFromIGDB.ts      # process only first 50
 */
...
/** Atari Jaguar IGDB platform ID */
const PLATFORM_ID = 62;
...
async function main() {
  console.log('🎮 Atari Jaguar IGDB platform sync');
  ...
  console.log(`✅ Release dates fetched (${rdMap.size} games have Jaguar dates)\n`);
