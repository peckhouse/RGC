/**
 * IGDB platform sync: fetches all non-cancelled Nintendo 3DS and New Nintendo
 * 3DS games from IGDB and upserts one DB row per (game × region) for EU / NA / JP.
 *
 * Nintendo 3DS (platform 37) and New Nintendo 3DS (platform 137) are treated
 * as a single platform group — games from both are deduped by IGDB game ID.
 * A small number of titles are New 3DS exclusives (e.g. Xenoblade Chronicles 3D);
 * fetching both platforms ensures they are included.
 ...
 *
 * Usage:
 *   npx tsx src/sync3DSFromIGDB.ts              # dry-run (no DB writes)
 *   DRY_RUN=false npx tsx src/sync3DSFromIGDB.ts # upsert to Supabase
 *   LIMIT=50 npx tsx src/sync3DSFromIGDB.ts      # process only first 50
 *
...
/** Nintendo 3DS IGDB platform ID */
const N3DS_PLATFORM_ID = 37;
/** New Nintendo 3DS IGDB platform ID */
const NEW_N3DS_PLATFORM_ID = 137;
const PLATFORM_IDS = [N3DS_PLATFORM_ID, NEW_N3DS_PLATFORM_ID];
...
async function main() {
  console.log('🎮 Nintendo 3DS / New Nintendo 3DS IGDB platform sync');
  ...
  //    New 3DS exclusives appear only under platform 137 and would be missed
  //    if we queried platform 37 alone.
  ...
  const platformLabels: Record<number, string> = {
    [N3DS_PLATFORM_ID]: 'Nintendo 3DS',
    [NEW_N3DS_PLATFORM_ID]: 'New Nintendo 3DS',
  };
  ...
  console.log(`📊 Total unique games (3DS + New 3DS combined): ${games.length}\n`);
  ...
  for (const game of games) {
    ...
    // First date encountered wins per region (merged from 3DS + New 3DS).
