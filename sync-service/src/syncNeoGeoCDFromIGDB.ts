/**
 * IGDB platform sync: fetches all non-cancelled Neo Geo CD games from IGDB
 * and upserts one DB row per (game x region) for EU / NA / JP whenever IGDB
 * has a regional release date for that combination.
 *
 * Usage:
 *   npx tsx src/syncNeoGeoCDFromIGDB.ts              # dry-run (no DB writes)
 *   DRY_RUN=false npx tsx src/syncNeoGeoCDFromIGDB.ts # upsert to Supabase
 *   LIMIT=50 npx tsx src/syncNeoGeoCDFromIGDB.ts      # process only first 50
 */
...
/** Neo Geo CD IGDB platform ID */
const PLATFORM_ID = 136;
...
async function main() {
  console.log('🎮 Neo Geo CD IGDB platform sync');
  ...
  console.log(`✅ Release dates fetched (${rdMap.size} games have Neo Geo CD dates)\n`);
