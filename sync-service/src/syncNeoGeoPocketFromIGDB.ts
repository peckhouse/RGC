/**
 * IGDB platform sync: fetches all non-cancelled Neo Geo Pocket and Neo Geo
 * Pocket Color games from IGDB and upserts one DB row per (game × region)
 * for EU / NA / JP.
 *
 * Neo Geo Pocket (platform 119) and Neo Geo Pocket Color (platform 120) are
 * treated as a single platform group — games from both are deduped by IGDB
 * game ID. Release dates are collected from both platform IDs and merged.
 *
 * Usage:
 *   npx tsx src/syncNeoGeoPocketFromIGDB.ts              # dry-run (no DB writes)
 *   DRY_RUN=false npx tsx src/syncNeoGeoPocketFromIGDB.ts # upsert to Supabase
 *   LIMIT=50 npx tsx src/syncNeoGeoPocketFromIGDB.ts      # process only first 50
 */
...
/** Neo Geo Pocket IGDB platform ID */
const NGP_PLATFORM_ID = 119;
/** Neo Geo Pocket Color IGDB platform ID */
const NGPC_PLATFORM_ID = 120;
const PLATFORM_IDS = [NGP_PLATFORM_ID, NGPC_PLATFORM_ID];
...
async function main() {
  console.log('🎮 Neo Geo Pocket / Pocket Color IGDB platform sync');
  ...
  const platformLabels: Record<number, string> = {
    [NGP_PLATFORM_ID]:  'Neo Geo Pocket',
    [NGPC_PLATFORM_ID]: 'Neo Geo Pocket Color',
  };
  ...
  console.log(`📊 Total unique games (Neo Geo Pocket + Pocket Color combined): ${games.length}\n`);
