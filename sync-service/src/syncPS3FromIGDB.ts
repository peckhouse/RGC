/**
 * IGDB platform sync: fetches all non-cancelled PlayStation 3 games from IGDB
 * and upserts one DB row per (game × region) for EU / NA / JP whenever IGDB
 * has a regional release date for that combination.
 *
 * Usage:
 *   npx tsx src/syncPS3FromIGDB.ts              # dry-run (no DB writes)
 *   DRY_RUN=false npx tsx src/syncPS3FromIGDB.ts # upsert to Supabase
 *   LIMIT=50 npx tsx src/syncPS3FromIGDB.ts      # process only first 50
 */
...
/** PlayStation 3 IGDB platform ID */
const PLATFORM_ID = 9;
...
async function main() {
  console.log('🎮 PlayStation 3 IGDB platform sync');
  ...
  console.log(`✅ Release dates fetched (${rdMap.size} games have PS3 dates)\n`);
