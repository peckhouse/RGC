/**
 * IGDB platform sync: fetches all non-cancelled TurboGrafx-16 and PC Engine
 * games from IGDB and upserts one DB row per (game × region) for EU / NA / JP.
 *
 * TurboGrafx-16 (platform 86) is the North American release and PC Engine
 * (platform 74) is the Japanese/European release of the same hardware. Both are
 * merged into the "TurboGrafx-16 / PC Engine" console via PLATFORM_ALIASES in
 * syncConsoles.ts. Games from both are deduped by IGDB game ID.
 * Release dates are collected from both platform IDs and merged.
 *
 * Usage:
 *   npx tsx src/syncTurboGrafx16FromIGDB.ts              # dry-run (no DB writes)
 *   DRY_RUN=false npx tsx src/syncTurboGrafx16FromIGDB.ts # upsert to Supabase
 *   LIMIT=50 npx tsx src/syncTurboGrafx16FromIGDB.ts      # process only first 50
 */
...
/** TurboGrafx-16 IGDB platform ID */
const TG16_PLATFORM_ID = 86;
/** PC Engine IGDB platform ID */
const PCE_PLATFORM_ID = 74;
const PLATFORM_IDS = [TG16_PLATFORM_ID, PCE_PLATFORM_ID];
...
async function main() {
  console.log('🎮 TurboGrafx-16 / PC Engine IGDB platform sync');
  ...
  const platformLabels: Record<number, string> = {
    [TG16_PLATFORM_ID]: 'TurboGrafx-16',
    [PCE_PLATFORM_ID]:  'PC Engine',
  };
  ...
  console.log(`📊 Total unique games (TurboGrafx-16 + PC Engine combined): ${games.length}\n`);
