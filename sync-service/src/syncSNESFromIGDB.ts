/**
 * IGDB platform sync: fetches all non-cancelled SNES and Super Famicom games
 * from IGDB and upserts one DB row per (game × region) for EU / NA / JP.
 *
 * Super Nintendo Entertainment System (platform 19) and Super Famicom
 * (platform 58) are treated as a single platform group — games from both are
 * deduped by IGDB game ID. Many JP-only titles are tagged exclusively under
 * the Super Famicom platform and would be missed with platform 19 alone.
 ...
 *
 * Usage:
 *   npx tsx src/syncSNESFromIGDB.ts              # dry-run (no DB writes)
 *   DRY_RUN=false npx tsx src/syncSNESFromIGDB.ts # upsert to Supabase
 *   LIMIT=50 npx tsx src/syncSNESFromIGDB.ts      # process only first 50
 *
...
/** Super Nintendo Entertainment System IGDB platform ID */
const SNES_PLATFORM_ID = 19;
/** Super Famicom IGDB platform ID */
const SFC_PLATFORM_ID = 58;
const PLATFORM_IDS = [SNES_PLATFORM_ID, SFC_PLATFORM_ID];
...
async function main() {
  console.log('🎮 SNES / Super Famicom IGDB platform sync');
  ...
  // 1. Paginate through games for both platforms; deduplicate by IGDB game ID.
  //    Many JP-only Super Famicom titles are tagged exclusively under platform 58
  //    and would be missed if we queried platform 19 alone.
  ...
  const platformLabels: Record<number, string> = {
    [SNES_PLATFORM_ID]: 'SNES',
    [SFC_PLATFORM_ID]: 'Super Famicom',
  };
  ...
  console.log(`📊 Total unique games (SNES + Super Famicom combined): ${games.length}\n`);

  const gameIds = games.map(g => g.id);

  // 2. Fetch release dates for both platform IDs and merge into a single rdMap.
  //    First date wins per (game, region) pair.
  ...
  for (const game of games) {
    ...
    // Build a map of RegionCode → release date.
    // Only explicit EU (1), NA (2), JP (5) release dates create entries.
    // First date encountered wins per region (merged from SNES + Super Famicom).
