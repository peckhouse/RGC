/**
 * IGDB platform sync: fetches all non-cancelled N64 games from IGDB and
 ...
 *
 * Usage:
 *   npx tsx src/syncN64FromIGDB.ts              # dry-run (no DB writes)
 *   DRY_RUN=false npx tsx src/syncN64FromIGDB.ts # upsert to Supabase
 *   LIMIT=50 npx tsx src/syncN64FromIGDB.ts      # process only first 50
 *
...
async function main() {
  console.log('🎮 N64 IGDB platform sync');
  ...
  // 1. Paginate through all N64 games on IGDB
  ...
  // 2. Batch-fetch release_dates for N64 directly from the release_dates endpoint.
  // This is more reliable than inline expansion in the games query.
  ...
  console.log(`✅ Release dates fetched (${rdMap.size} games have N64 dates)\n`);
