/**
 * IGDB platform sync: fetches all non-cancelled Game Boy and Game Boy Color
 * games from IGDB and upserts one DB row per (game × region) for EU / NA / JP.
 *
 * Game Boy (platform 33) and Game Boy Color (platform 22) are treated as a
 * single platform group — games from both are deduped by IGDB game ID.
 ...
 *
 * Usage:
 *   npx tsx src/syncGBFromIGDB.ts              # dry-run (no DB writes)
 *   DRY_RUN=false npx tsx src/syncGBFromIGDB.ts # upsert to Supabase
 *   LIMIT=50 npx tsx src/syncGBFromIGDB.ts      # process only first 50
 *
...
/** Game Boy IGDB platform ID */
const GB_PLATFORM_ID = 33;
/** Game Boy Color IGDB platform ID */
const GBC_PLATFORM_ID = 22;
const PLATFORM_IDS = [GB_PLATFORM_ID, GBC_PLATFORM_ID];
...
async function main() {
  console.log('🎮 Game Boy / Game Boy Color IGDB platform sync');
  ...
  //    A game released on both GB and GBC (e.g. a dual-mode cart) appears once.
  ...
  const platformLabels: Record<number, string> = {
    [GB_PLATFORM_ID]: 'Game Boy',
    [GBC_PLATFORM_ID]: 'Game Boy Color',
  };
  ...
  console.log(`📊 Total unique games (GB + GBC combined): ${games.length}\n`);
  ...
  for (const game of games) {
    ...
    // First date encountered wins per region (merged from GB + GBC).
    ...
    for (const rd of rdMap.get(game.id) ?? []) {
      ...
      if (!code || regionDates.has(code)) continue; // skip unknown regions; first date wins
