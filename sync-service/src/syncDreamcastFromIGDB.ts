/**
 * IGDB platform sync: fetches all non-cancelled Dreamcast games from IGDB
 * and upserts one DB row per (game × region) for EU / NA / JP whenever IGDB
 * has a regional release date for that combination.
 *
 * Usage:
 *   npx tsx src/syncDreamcastFromIGDB.ts              # dry-run (no DB writes)
 *   DRY_RUN=false npx tsx src/syncDreamcastFromIGDB.ts # upsert to Supabase
 *   LIMIT=50 npx tsx src/syncDreamcastFromIGDB.ts      # process only first 50
 */
...
async function main() {
  console.log('🎮 Dreamcast IGDB platform sync');
  ...
  console.log(`✅ Release dates fetched (${rdMap.size} games have Dreamcast dates)\n`);
