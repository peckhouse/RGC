/**
 * IGDB platform sync: fetches all non-cancelled Wii U games from IGDB and
 ...
 *
 * Usage:
 *   npx tsx src/syncWiiUFromIGDB.ts              # dry-run (no DB writes)
 *   DRY_RUN=false npx tsx src/syncWiiUFromIGDB.ts # upsert to Supabase
 *   LIMIT=50 npx tsx src/syncWiiUFromIGDB.ts      # process only first 50
 *
...
/** Wii U IGDB platform ID */
const PLATFORM_ID = 41;
...
async function main() {
  console.log('🎮 Wii U IGDB platform sync');
  ...
  // 1. Paginate through all Wii U games on IGDB
  ...
  // 2. Batch-fetch release_dates for Wii U directly from the release_dates endpoint.
  ...
  console.log(`✅ Release dates fetched (${rdMap.size} games have Wii U dates)\n`);
