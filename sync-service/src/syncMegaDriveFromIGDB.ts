/**
 * IGDB platform sync: fetches all non-cancelled Sega Mega Drive / Genesis
 ...
 *
 * Usage:
 *   npx tsx src/syncMegaDriveFromIGDB.ts              # dry-run (no DB writes)
 *   DRY_RUN=false npx tsx src/syncMegaDriveFromIGDB.ts # upsert to Supabase
 *   LIMIT=50 npx tsx src/syncMegaDriveFromIGDB.ts      # process only first 50
 *
...
/** Sega Mega Drive / Genesis IGDB platform ID */
const PLATFORM_ID = 29;
...
async function main() {
  console.log('🎮 Sega Mega Drive / Genesis IGDB platform sync');
  ...
  console.log(`✅ Release dates fetched (${rdMap.size} games have Mega Drive dates)\n`);
