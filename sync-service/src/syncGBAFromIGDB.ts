/**
 * IGDB platform sync: fetches all non-cancelled Game Boy Advance games from
 * IGDB and upserts one DB row per (game × region) for EU / NA / JP whenever
 * IGDB has a regional release date for that combination.
 *
 * Cover art priority:
 ...
 *
 * Usage:
 *   npx tsx src/syncGBAFromIGDB.ts              # dry-run (no DB writes)
 *   DRY_RUN=false npx tsx src/syncGBAFromIGDB.ts # upsert to Supabase
 *   LIMIT=50 npx tsx src/syncGBAFromIGDB.ts      # process only first 50
 *
...
/** Game Boy Advance IGDB platform ID */
const PLATFORM_ID = 24;
...
async function main() {
  console.log('🎮 Game Boy Advance IGDB platform sync');
  ...
  // 1. Paginate through all GBA games on IGDB
  ...
  // 2. Batch-fetch release_dates for GBA directly from the release_dates endpoint.
  ...
  console.log(`✅ Release dates fetched (${rdMap.size} games have GBA dates)\n`);
