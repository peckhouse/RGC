/**
 * IGDB platform sync: fetches all non-cancelled Game Gear games from IGDB
 * and upserts one DB row per (game × region) for EU / NA / JP whenever IGDB
 * has a regional release date for that combination.
 *
 * Usage:
 *   npx tsx src/syncGameGearFromIGDB.ts              # dry-run (no DB writes)
 *   DRY_RUN=false npx tsx src/syncGameGearFromIGDB.ts # upsert to Supabase
 *   LIMIT=50 npx tsx src/syncGameGearFromIGDB.ts      # process only first 50
 */
...
/** Game Gear IGDB platform ID */
const PLATFORM_ID = 35;
...
async function main() {
  console.log('🎮 Game Gear IGDB platform sync');
  ...
  console.log(`✅ Release dates fetched (${rdMap.size} games have Game Gear dates)\n`);
