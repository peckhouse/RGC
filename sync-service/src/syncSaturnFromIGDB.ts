/**
 * IGDB platform sync: fetches all non-cancelled Sega Saturn games from IGDB
 * and upserts one DB row per (game × region) for EU / NA / JP whenever IGDB
 * has a regional release date for that combination.
 *
 * Usage:
 *   npx tsx src/syncSaturnFromIGDB.ts              # dry-run (no DB writes)
 *   DRY_RUN=false npx tsx src/syncSaturnFromIGDB.ts # upsert to Supabase
 *   LIMIT=50 npx tsx src/syncSaturnFromIGDB.ts      # process only first 50
 */
...
async function main() {
  console.log('🎮 Sega Saturn IGDB platform sync');
  ...
  console.log(`✅ Release dates fetched (${rdMap.size} games have Saturn dates)\n`);
