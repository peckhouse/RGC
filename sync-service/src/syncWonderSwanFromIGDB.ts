/**
 * IGDB platform sync: fetches all non-cancelled WonderSwan and WonderSwan Color
 * games from IGDB and upserts one DB row per (game × region) for EU / NA / JP.
 *
 * WonderSwan (platform 57) and WonderSwan Color (platform 123) are treated as a
 ...
 *
 * ⚠️  Verify platform IDs via dry run: if WonderSwan Color games are missing,
 *     check the correct IGDB ID for "WonderSwan Color" and update NGPC_PLATFORM_ID.
 *
 * Usage:
 *   npx tsx src/syncWonderSwanFromIGDB.ts              # dry-run (no DB writes)
 *   DRY_RUN=false npx tsx src/syncWonderSwanFromIGDB.ts # upsert to Supabase
 *   LIMIT=50 npx tsx src/syncWonderSwanFromIGDB.ts      # process only first 50
 */
...
/** WonderSwan IGDB platform ID */
const WS_PLATFORM_ID = 57;
/** WonderSwan Color IGDB platform ID */
const WSC_PLATFORM_ID = 123;
const PLATFORM_IDS = [WS_PLATFORM_ID, WSC_PLATFORM_ID];
...
async function main() {
  console.log('🎮 WonderSwan / WonderSwan Color IGDB platform sync');
  ...
  const platformLabels: Record<number, string> = {
    [WS_PLATFORM_ID]:  'WonderSwan',
    [WSC_PLATFORM_ID]: 'WonderSwan Color',
  };
  ...
  console.log(`📊 Total unique games (WonderSwan + WonderSwan Color combined): ${games.length}\n`);
